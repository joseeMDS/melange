...
include Record_intf.Make(struct
  ...
  let rec mk_structural_ty all_types (core_type, supported)  = match core_type.ptyp_desc with
    ...
    |Ptyp_tuple(types) -> 
      ...
      let pexp_fun = Builder.pexp_fun ~loc Nolabel None in
      (* let body = List.filter_map (fun type_ -> mk_body_apply all_types (type_, Supported(":)")) "lesgo" ) types in *)
      (* let acc::types = types in *)
      (* TODO: map over args and call mk_body_apply with arg number and tuple equivalent of that arg *)

      let body = List.fold_left2 (
        fun acc type_ arg ->
          match (mk_body_apply all_types (type_, (find_core_type_support type_ all_types)) arg) with
          | None -> acc
          | Some(expr) -> pexp_fun (Builder.ppat_var ~loc @@ with_loc arg) expr
      ) (pexp_fun (List.nth args_pat 0)  [%expr st] ) types args in
    
      {
        eta = [%expr fun self st [%p tuple_pattern] -> [%e body]];
        (* Code not called *)
    ...
    let module SSet = Set.Make(String) in
      ...
      let build_iter =
        ...
        let build_super = 
          ...
          let body = List.map (mk_method all_types) all_types
            in
